...
{
    public sealed class NonAdaptedTask : Tasking
    {
        double a_ha;
        protected override void Initials()
        {
            base.Initials();
            var grad = (Math.Pow(hm * (Size - 1), 2) * 2 * Math.Log(hm * (Size - 1) / ModelElement.r0) + Math.Pow(hm * (Size - 1), 2) + Math.Pow(ModelElement.r0, 2)) * modelData.mrr / (6000 * 4 * ModelElement.alfa * ModelElement.d);
